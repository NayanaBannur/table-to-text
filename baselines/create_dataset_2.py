...
train_size_per_rule = [2000]
dev_size_per_rule = [100]
test_size_per_rule = [100]
...
def one_row_largest(datum):
    ...
    data = np.random.uniform(min_num, max_num, (datum["num_rows"], datum["num_cols"])).astype('int')
...
def rows_comp(datum):
    ...
    datum["table_caption"] = f"Table shows {first_rh} and {second_rh} for {col_name}"
    ...
    if first < second:
        datum["text"] = f"{second_rh} is greater than {first_rh} for {col_name}"
    else:
        datum["text"] = f"{first_rh} is greater than {second_rh} for {col_name}"
...
def cols_comp(datum):
    ...
    datum["table_caption"] = f"Table shows {first_ch} and {second_ch} for {row_name}"
    ...
    diff = int(np.round(np.abs(first-second)))
    if first < second:
        datum["text"] = f"{second_ch} is greater than {first_ch} for {row_name}"
    else:
        datum["text"] = f"{first_ch} is greater than {second_ch} for {row_name}"
...
values = list(train_papers.values())
keys = list(train_papers.keys())
random.shuffle(values)

train_papers = {k:v for k,v in zip(keys, values)}
